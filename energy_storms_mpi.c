...
float update( float *layer, int layer_size, int k, int pos, float energy ) {
...
int main(int argc, char *argv[]) {
    ...
    for( i=0; i<num_storms; i++) {
        ...
        for( j=0; j<storms[i].size; j++ ) {
            ...
            if (rank == 0){
                energy_value[0] = update( layer, layer_size, 0, position, energy);
                layer[0] = layer[0] + energy_value[0];
                /* For each cell in the layer */
                for( k=1; k<layer_size; k++ ) {
                    /* Update the energy value for the cell */
                    MPI_Recv(energy_value, 1, MPI_FLOAT, k, k, MPI_COMM_WORLD, &status);
                    layer[k] = layer[k] + energy_value[0];
                }
            }
            else if (rank > layer_size - 1){
                break;
            }
            else {
                
                energy_value[0] = update( layer, layer_size, rank, position, energy );
                MPI_Send(energy_value, 1, MPI_FLOAT, 0, rank, MPI_COMM_WORLD);
