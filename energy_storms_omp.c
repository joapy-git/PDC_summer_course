...
int main(int argc, char *argv[]) {
    ...
    #pragma omp parallel for default(none) shared(layer_size, layer, layer_copy) private(k)
    for( k=0; k<layer_size; k++ ) layer[k] = 0.0f;
    #pragma omp parallel for default(none) shared(layer_size, layer_copy) private(k)
    ...
    for( i=0; i<num_storms; i++) {
        ...
        /* For each particle */
        #pragma omp parallel for default(none) shared(storms, layer_size, i) private(j, k, energy, position) reduction(+:layer[:layer_size])
        ...
        #pragma omp parallel for default(none) shared(layer, layer_copy, layer_size) private(k)
        for( k=0; k<layer_size; k++ ) 
            layer_copy[k] = layer[k];

        /* 4.2.2. Update layer using the ancillary values.
                  Skip updating the first and last positions */
        #pragma omp parallel for default(none) shared(layer_copy, layer, layer_size) private(k)
        for( k=1; k<layer_size-1; k++ )
            layer[k] = ( layer_copy[k-1] + layer_copy[k] + layer_copy[k+1] ) / 3;

        /* 4.3. Locate the maximum value in the layer, and its position */
        #pragma omp parallel for default(none) shared(i, layer, layer_size, maximum, positions) private(k)
        for( k=1; k<layer_size-1; k++ ) {
